...
def isap_k_128a_encrypt(
    ...
    M -bytes cipher text & 16 -bytes authentication tag ( in order )
    """
    assert len(key) == 16, "ISAP-K-128A takes 16 -bytes secret key !"
    assert len(nonce) == 16, "ISAP-K-128A takes 16 -bytes nonce !"
    ...
    args = [uint8_tp, uint8_tp, uint8_tp, len_t,
            uint8_tp, uint8_tp, len_t, uint8_tp]
    SO_LIB.isap_k_128a_encrypt.argtypes = args

    SO_LIB.isap_k_128a_encrypt(key_, nonce_, data_, ad_len,
                               text_, enc, ct_len, tag)
...
def isap_k_128a_decrypt(
    ...
    assert len(key) == 16, "ISAP-K-128A takes 16 -bytes secret key !"
    assert len(nonce) == 16, "ISAP-K-128A takes 16 -bytes nonce !"
    assert len(tag) == 16, "ISAP-K-128A takes 16 -bytes authentication tag !"
    ...
    args = [uint8_tp, uint8_tp, uint8_tp,
            uint8_tp, len_t, uint8_tp, uint8_tp, len_t]
    SO_LIB.isap_k_128a_decrypt.argtypes = args
    SO_LIB.isap_k_128a_decrypt.restype = bool_t

    f = SO_LIB.isap_k_128a_decrypt(key_, nonce_, tag_, data_,
                                   ad_len, enc_, dec, ct_len)
...
def isap_k_128_encrypt(
    ...
    M -bytes cipher text & 16 -bytes authentication tag ( in order )
    """
    assert len(key) == 16, "ISAP-K-128 takes 16 -bytes secret key !"
    assert len(nonce) == 16, "ISAP-K-128 takes 16 -bytes nonce !"
    ...
    args = [uint8_tp, uint8_tp, uint8_tp, len_t,
            uint8_tp, uint8_tp, len_t, uint8_tp]
    SO_LIB.isap_k_128_encrypt.argtypes = args

    SO_LIB.isap_k_128_encrypt(key_, nonce_, data_, ad_len,
                              text_, enc, ct_len, tag)
...
def isap_k_128_decrypt(
    ...
    assert len(key) == 16, "ISAP-K-128 takes 16 -bytes secret key !"
    assert len(nonce) == 16, "ISAP-K-128 takes 16 -bytes nonce !"
    assert len(tag) == 16, "ISAP-K-128 takes 16 -bytes authentication tag !"
    ...
    args = [uint8_tp, uint8_tp, uint8_tp,
            uint8_tp, len_t, uint8_tp, uint8_tp, len_t]
    SO_LIB.isap_k_128_decrypt.argtypes = args
    SO_LIB.isap_k_128_decrypt.restype = bool_t

    f = SO_LIB.isap_k_128_decrypt(key_, nonce_, tag_, data_,
                                  ad_len, enc_, dec, ct_len)
